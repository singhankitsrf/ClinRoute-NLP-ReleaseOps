...
STATE: dict[str, NLPService | None] = {"service": None}
...
def build_service() -> NLPService | None:
    ...
    try:
        if os.path.exists(os.path.join(model_dir, "metadata.json")):
            ...
            predictor = BaselineBundle.load(model_dir)
    except Exception:
        return None

    return NLPService(
        predictor,
        redact=os.getenv("REDACT_IDENTIFIERS", "true").lower() == "true",
        route_min_confidence=float(os.getenv("ROUTE_MIN_CONFIDENCE", "0.70")),
        urgency_min_confidence=float(os.getenv("URGENCY_MIN_CONFIDENCE", "0.70")),
    )
...
app = FastAPI(
    title="ClinRoute NLP ReleaseOps API",
    version="0.1.0",
    description="NLP routing and urgency prediction with release-quality guardrails.",
    lifespan=lifespan,
)
